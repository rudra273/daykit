...
class ExpenseRepository(
     ...
     *
     * Every table here keys its primary key on the autogenerated row `id`, but a
     * backup only carries the stable business key (`billId` / `entryId` / …). An
     * entity rebuilt from a backup therefore has `id = 0`, which `@Upsert` treats
     * as a fresh insert — producing a duplicate that then violates the unique
     * index. Looking up the existing row and reusing its `id` turns each write
     * back into a true update.
     *
     * `expense_months` is exempt: its primary key already *is* `monthKey`.
     */
    suspend fun importRecords(record: ExpenseBackupRecord) {
        record.months.forEach { dao.upsertMonth(it) }
        record.bills.forEach { bill ->
            val existing = dao.getBill(bill.billId)
            dao.upsertBill(
                MonthlyBillEntity(
                    id = existing?.id ?: 0,
                    billId = bill.billId,
        ...
        record.billAmounts.forEach { change ->
            // Uses INSERT OR REPLACE against the unique (billId, effectiveMonthKey)
            // index, so this one already replaces in place.
            dao.upsertBillAmount(change.toEntity())
        }
        record.entries.forEach { entry ->
            val existing = dao.getEntry(entry.entryId)
            dao.upsertEntry(entry.toEntity().copy(id = existing?.id ?: 0))
        }
    }
